...
def aplicar_bordas_celula(cell):
    from docx.oxml import OxmlElement
                ...
                if par.startswith("1.2.") or (len(cl["conteudo"]) == 2 and idx_p == 0):
                    ...
                    if tab_din and tab_din.get("headers") and tab_din.get("rows"):
                        ...
                        for c_idx, h_text in enumerate(dyn_headers):
                            ...
                            aplicar_bordas_celula(hdr_cells[c_idx])
                        ...
                        for r_idx, row_vals in enumerate(dyn_rows):
                            ...
                            bg_color = 'E2EBF4' if (r_idx + 1) % 2 == 0 else 'FFFFFF'
                            for c in row_cells:
                                aplicar_fundo_celula_docx(c, bg_color)
                            for c_idx, cell_val in enumerate(row_vals):
                                if c_idx < len(row_cells):
                                    row_cells[c_idx].text = str(cell_val)
                                    aplicar_bordas_celula(row_cells[c_idx])
                        ...
                        for i_h, h in enumerate(headers):
                            ...
                            aplicar_bordas_celula(hdr_cells[i_h])
                        ...
                        for idx_it, row_it in enumerate(itens_list):
                            ...
                            bg_color = 'E2EBF4' if (idx_it + 1) % 2 == 0 else 'FFFFFF'
                            for c in r_cells:
                                aplicar_fundo_celula_docx(c, bg_color)
                            vals = [
                            ...
                            for c_i, v in enumerate(vals):
                                r_cells[c_i].text = v
                                aplicar_bordas_celula(r_cells[c_i])
            ...
            for idx_d, par in enumerate(cl["conteudo"]):
                ...
                if idx_d == 0:
                    ...
                    for r_i, (lbl, val) in enumerate(dot_items):
                        ...
                        # Col 1 is ALWAYS Blue. Col 2 & 3 alternate (Blue header, then White, Blue, White...)
                        aplicar_fundo_celula_docx(c0, 'E2EBF4')
                        bg_23 = 'E2EBF4' if r_i % 2 == 0 else 'FFFFFF'
                        aplicar_fundo_celula_docx(c1, bg_23)
                        aplicar_fundo_celula_docx(c2, bg_23)
                        
                        if r_i == 0:
                            c1.merge(c2)
                        
                        aplicar_bordas_celula(c0)
                        aplicar_bordas_celula(c1)
                        if r_i != 0:
                            aplicar_bordas_celula(c2)
                        ...
                        r1.font.name = "Arial"
                        r1.font.size = Pt(8.5)
                        r1.font.color.rgb = RGBColor(34, 64, 97)
                        ...
                        if r_i == 0:
                            ...
                            r2.font.name = "Arial"
                            r2.font.size = Pt(8.5)
                            r2.font.color.rgb = RGBColor(34, 64, 97)
                ...
                if idx_d == 0:
                    ...
                    for k, v in dot_items:
                        ...
                        t_dot_data.append([
                            ...
                            Paragraph(f"<font color='#224061'>{pv1}</font>", style_tbl_td), 
                            Paragraph(f"<font color='#224061'>{pv2}</font>", style_tbl_td)
                        ])
                    t_dot = Table(t_dot_data, colWidths=[140, 220, 120])
