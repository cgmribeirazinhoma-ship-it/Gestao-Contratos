...
def extrair_tabela_dinamica_df(df) -> dict:
    ...
    try:
        ...
        for i, h in enumerate(headers):
            h_up = h.upper()
            if any(k in h_up for k in ['V. TOTAL', 'VL. TOTAL', 'PREÇO TOTAL', 'VALOR TOTAL', 'SUBTOTAL', 'TOTAL']) and 'UNIT' not in h_up:
                total_col_idx = i
                break
        ...
        if total_col_idx is not None:
            ...
            for i in reversed(range(num_cols)):
                ...
                if any(k in h_up for k in ['VALOR', 'PREÇO', 'PRECO']):
                    total_col_idx = i
                    break
            if total_col_idx is not None:
                for r in rows:
    ...
    try:
        ...
        for col in df.columns:
            ...
            if any(k in c_upper for k in ['UNIT', 'UNITARIO', 'V. UNIT', 'VL. UNIT', 'PREÇO UNIT', 'VALOR UNITÁRIO']):
                col_vu = col
            elif any(k in c_upper for k in ['V. TOTAL', 'VL. TOTAL', 'PREÇO TOTAL', 'VALOR TOTAL', 'TOTAL']):
                col_vt = col
