...
def criar_modelo_excel_itens() -> bytes:
    """Gera uma planilha Excel (.xlsx) modelo padrão para importação de itens da contratação"""
    import pandas as pd
    dados_exemplo = [
        {"ITEM": 1, "DESCRIÇÃO": "Oxford Preto", "QUANTIDADE": 945, "UNID.": "Metros", "VALOR UNITÁRIO": 14.13, "VALOR TOTAL": 13352.85},
        {"ITEM": 2, "DESCRIÇÃO": "Oxford Preto Premium", "QUANTIDADE": 300, "UNID.": "Metros", "VALOR UNITÁRIO": 14.13, "VALOR TOTAL": 4239.00},
        {"ITEM": 3, "DESCRIÇÃO": "Oxford Laranja", "QUANTIDADE": 90, "UNID.": "Metros", "VALOR UNITÁRIO": 17.86, "VALOR TOTAL": 1607.40},
        {"ITEM": 4, "DESCRIÇÃO": "Oxford Azul Royal", "QUANTIDADE": 50, "UNID.": "Metros", "VALOR UNITÁRIO": 9.02, "VALOR TOTAL": 451.00},
        {"ITEM": 5, "DESCRIÇÃO": "Oxford Azul Royal", "QUANTIDADE": 250, "UNID.": "Metros", "VALOR UNITÁRIO": 9.02, "VALOR TOTAL": 2255.00}
    ]
    df = pd.DataFrame(dados_exemplo)
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='Itens do Contrato')
    out.seek(0)
    return out.getvalue()
...
def extrair_tabela_dinamica_df(df) -> dict:
    ...
    try:
        ...
        descartes_total = ['QTD', 'QUANT', 'QUANTIDADE', 'DIAS', 'HORAS', 'PARCELA', 'MESES', 'PAGAMENTO', 'UNID', 'UNIT']
        ...
        for i, h in enumerate(headers):
            h_up = h.upper()
            if any(k in h_up for k in ['QTD', 'QUANT', 'QUANTIDADE']) and qtd_col_idx is None:
                qtd_col_idx = i
            elif any(k in h_up for k in ['UNIT', 'UNITARIO', 'V. UNIT', 'VL. UNIT', 'PREÇO UNIT', 'VALOR UNIT']) and vu_col_idx is None:
                vu_col_idx = i

        def _is_summary_row(row_vals):
            for cell in row_vals[:3]:
        ...
        for r in rows:
            if _is_summary_row(r):
                if total_col_idx is not None and total_col_idx < len(r):
                    resumo_row_val = _parse_num_local(r[total_col_idx])
            else:
                item_rows.append(r)
        ...
        elif resumo_row_val and resumo_row_val > 0 and abs(total_geral - resumo_row_val) < 0.05:
            total_geral = resumo_row_val
...
def processar_planilha_itens_excel(file_bytes_or_buffer, file_name: str = "", objeto_contexto: str = "") -> ResultadoPlanilhaItens:
    ...
    try:
                        ...
                        for t in tables:
                            for r in t:
                                if any(cell and str(cell).strip() for cell in r):
                                    all_rows.append([str(cell or '').strip() for cell in r])
                    
                    if len(all_rows) >= 2:
                        headers = [h if h else f"COL_{i+1}" for i, h in enumerate(all_rows[0])]
                        data_rows = []
                        for r in all_rows[1:]:
        ...
        descartes_vt = ['QTD', 'QUANT', 'QUANTIDADE', 'DIAS', 'HORAS', 'PARCELA', 'MESES', 'PAGAMENTO', 'UNID', 'UNIT']
        for col in df.columns:
            c_upper = str(col).strip().upper()
            if any(k in c_upper for k in ['UNIT', 'UNITARIO', 'V. UNIT', 'VL. UNIT', 'PREÇO UNIT', 'VALOR UNITÁRIO']):
                col_vu = col
                ...
                col_vt = col
            elif any(k in c_upper for k in ['QTD', 'QUANT', 'QUANTIDADE']):
                col_qtd = col
            elif any(k in c_upper for k in ['UNIDADE', 'UNID', 'UND', 'MEDIDA']) and 'UNIT' not in c_upper:
                col_unid = col
        ...
        for idx, row in df.iterrows():
            ...
            row_str_full = " ".join([str(v).upper().strip() for v in row.values if pd.notna(v)])
            first_cell = str(row.values[0]).upper().strip() if len(row.values) > 0 and pd.notna(row.values[0]) else ""
            if any(k in row_str_full for k in ["TOTAL GERAL", "VALOR TOTAL", "SUBTOTAL", "TOTAL DA PROPOSTA", "SOMA TOTAL"]) or row_str_full.startswith(("TOTAL", "SUBTOTAL", "SOMA", "RESUMO")) or first_cell in ["TOTAL", "SUBTOTAL", "SOMA"]:
                continue
            ...
            item_num = int(limpar_num(row[col_item])) if col_item and not pd.isna(row[col_item]) else (idx + 1)
            ...
            vt = limpar_num(row[col_vt]) if col_vt else (qtd * vu)
        ...
        if soma_total_itens > 0 and isinstance(tab_din, dict):
            if not tab_din.get("total_geral") or tab_din.get("total_geral") == 0.0 or abs(tab_din.get("total_geral", 0.0) - soma_total_itens) > 0.01:
                tab_din["total_geral"] = soma_total_itens
                tab_din["total_formatado"] = formatar_moeda_br(soma_total_itens)
...
def gerar_clausulas_agu(dados: Dict[str, Any]) -> list:
    """Monta a lista estruturada das cláusulas contratuais oficiais da AGU (Lei 14.133/2021)"""
    dados = preparar_dados_contrato(dados)
    mod = dados["modelo_agu"]
    tipo_titulo = "AQUISIÇÃO DE BENS / FORNECIMENTO" if "COMPRA" in mod or "BEM" in mod else (
        "PRESTAÇÃO DE SERVIÇOS NÃO CONTÍNUOS" if "ESCOPO" in mod else (
            "OBRAS E SERVIÇOS DE ENGENHARIA" if "OBRA" in mod or "ENGENHARIA" in mod else "PRESTAÇÃO DE SERVIÇOS CONTÍNUOS"
        )
    )
    ...
    clausulas_base = [
        {
            "numero": "CLÁUSULA PRIMEIRA – OBJETO (art. 92, I e II)",
            "conteudo": [
                ...
                "1.3. O regime de execução contratual adotado observa integralmente as normas da Lei Federal nº 14.133, de 1º de abril de 2021, e seus regulamentos correlatos no âmbito municipal."
            ]
        },
        {
            "numero": "CLÁUSULA SEGUNDA – DA VIGÊNCIA E PRORROGAÇÃO",
            "conteudo": [
        ...
        {
            "numero": "CLÁUSULA TERCEIRA – DOS PREÇOS E DO VALOR GLOBAL",
            "conteudo": [
                f"3.1. O valor total global deste Contrato é de {dados['valor_total_formatado']} ({dados['valor_extenso']}).",
                "3.2. No valor acima estão incluídas todas as despesas ordinárias diretas e indiretas decorrentes da execução do objeto, inclusive tributos, encargos sociais, trabalhistas, previdenciários, fiscais e comerciais, fretes, seguros e quaisquer outros que incidam ou venham a incidir sobre o cumprimento do ajuste."
            ]
        },
        {
            "numero": "CLÁUSULA QUARTA – DA DOTAÇÃO ORÇAMENTÁRIA",
            "conteudo": [
                f"4.1. As despesas decorrentes da presente contratação correrão à conta dos recursos orçamentários específicos da(o) {dados['orgao_nome']}, conforme a seguinte classificação orçamentária:",
                f"    {dados['dotacao_orcamentaria']}",
                "4.2. A dotação relativa aos exercícios financeiros subsequentes será indicada após a aprovação da respectiva Lei Orçamentária Anual, mediante simples apostilamento."
            ]
        },
        {
            "numero": "CLÁUSULA QUINTA – DO PAGAMENTO E LIQUIDAÇÃO",
            "conteudo": [
                "5.1. O pagamento será realizado no prazo de até 30 (trinta) dias consecutivos contados do recebimento definitivo do objeto ou serviço e da apresentação da respectiva Nota Fiscal/Fatura devidamente atestada pelo Fiscal do Contrato.",
                "5.2. A liquidação da despesa observará rigorosamente a ordem cronológica para cada fonte diferenciada de recursos, nos moldes do art. 141 da Lei Federal nº 14.133/2021.",
                "5.3. Nenhum pagamento será efetuado enquanto pendente de liquidação qualquer obrigação financeira ou apresentação de regularidade fiscal e trabalhista por parte da CONTRATADA."
            ]
        },
        {
            "numero": "CLÁUSULA SEXTA – DO REAJUSTE DE PREÇOS",
            "conteudo": [
                "6.1. Os preços são fixos e irreajustáveis pelo período de 12 (doze) meses contados da data limite para apresentação da proposta ou do orçamento estimado.",
                "6.2. Após o interregno de 1 (um) ano, os preços poderão ser reajustados mediante aplicação do Índice Nacional de Preços ao Consumidor Amplo (IPCA/IBGE) ou índice setorial oficial correspondente, exclusivamente para compensar a variação efetiva dos custos de produção."
            ]
        },
        {
            "numero": "CLÁUSULA SÉTIMA – DA FISCALIZAÇÃO E RECEBIMENTO DO OBJETO",
            "conteudo": [
                f"7.1. A execução do presente contrato será acompanhada e fiscalizada pelo servidor {dados['fiscal']}, formalmente designado como Fiscal do Contrato pela autoridade competente, nos termos do art. 117 da Lei nº 14.133/2021.",
                "7.2. O recebimento provisório dar-se-á pelo fiscal no ato da entrega ou término da etapa, para verificação de conformidade.",
                "7.3. O recebimento definitivo será formalizado no prazo de até 15 (quinze) dias após o recebimento provisório, mediante termo detalhado emitido pela comissão ou gestor competente."
            ]
        },
        {
            "numero": "CLÁUSULA OITAVA – DAS OBRIGAÇÕES DA CONTRATANTE",
            "conteudo": [
                "8.1. Exigir o cumprimento de todas as obrigações assumidas pela CONTRATADA, de acordo com as cláusulas contratuais e os termos de sua proposta.",
                "8.2. Proporcionar todas as condições necessárias para que a CONTRATADA possa desempenhar regularmente o fornecimento ou a prestação dos serviços contratados.",
                "8.3. Efetuar o pagamento à CONTRATADA no valor e prazo pactuados, após o regular atesto da execução.",
                "8.4. Notificar formalmente a CONTRATADA sobre quaisquer irregularidades, imperfeições ou falhas constatadas na execução do objeto, fixando prazo razoável para a sua correção."
            ]
        },
        {
            "numero": "CLÁUSULA NONA – DAS OBRIGAÇÕES DA CONTRATADA",
            "conteudo": [
                "9.1. Executar o objeto contratado com presteza, perfeição e em estrita consonância com as especificações da proposta e do Termo de Referência.",
                "9.2. Manter, durante toda a vigência do contrato, em compatibilidade com as obrigações por ela assumidas, todas as condições de habilitação e qualificação exigidas na contratação.",
                ...
                "9.4. Responsabilizar-se pelos vícios e danos decorrentes da execução do objeto, bem como por todo e qualquer dano causado à Administração Pública ou a terceiros."
            ]
        },
        {
            "numero": "CLÁUSULA DÉCIMA – DAS INFRAÇÕES E SANÇÕES ADMINISTRATIVAS",
            "conteudo": [
                "10.1. O descumprimento total ou parcial das obrigações assumidas ensejará a aplicação das sanções administrativas previstas no art. 156 da Lei Federal nº 14.133/2021, garantida a prévia e ampla defesa:",
        ...
        {
            "numero": "CLÁUSULA DÉCIMA PRIMEIRA – DA EXTINÇÃO CONTRATUAL",
            "conteudo": [
                "11.1. O presente contrato poderá ser extinto nas hipóteses e condições previstas nos artigos 137, 138 e 139 da Lei Federal nº 14.133/2021, sem prejuízo da incidência das sanções cabíveis.",
                "11.2. A extinção determinada por ato unilateral da Administração Pública será precedida de autorização escrita e fundamentada da autoridade competente e de regular processo administrativo."
            ]
        },
        {
            "numero": "CLÁUSULA DÉCIMA SEGUNDA – DA LEI GERAL DE PROTEÇÃO DE DADOS (LGPD)",
            "conteudo": [
                "12.1. As partes comprometem-se a cumprir integralmente as disposições da Lei Federal nº 13.709/2018 (Lei Geral de Proteção de Dados Pessoais - LGPD), implementando medidas técnicas e administrativas aptas a proteger os dados pessoais acessados em razão deste contrato contra acessos não autorizados ou tratamento ilícito."
            ]
        }
    ]
    ...
    # Verifica se há cláusulas adicionais inteligentemente formatadas enviadas do frontend
    ...
    if clausulas_extras and isinstance(clausulas_extras, list) and len(clausulas_extras) > 0:
        # Inserir antes da Cláusula do Foro (que sempre deve ser a última)
        # O tamanho atual é 12, então a nova será DÉCIMA TERCEIRA, etc.
        idx_nova = len(clausulas_base)
        nome_ordinal = ordinais[idx_nova] if idx_nova < len(ordinais) else f"{idx_nova + 1}ª"
        
        # Título definido livremente pelo usuário ou padrão oficial
        ...
        if not titulo_usuario or not str(titulo_usuario).strip():
            ...
            # Remover repetição de "CLÁUSULA..." se o usuário tiver digitado no início
    ...
    clausulas_base.append({
        "numero": f"CLÁUSULA {nome_foro} – DO FORO",
        "conteudo": [
...
def gerar_contrato_docx(dados_brutos: Dict[str, Any]) -> io.BytesIO:
    ...
    r2 = p_id.add_run(f"{dados['modalidade'].upper()} Nº {dados['numero_modalidade']}\n")
    r2.bold = True
    r2.font.name = "Arial"
    r2.font.size = Pt(10)
    ...
    preambulo_texto = (
        ...
        f"Lei nº 14.133, de 1º de abril de 2021, resolvem celebrar o presente Termo de Contrato, decorrente do "
        f"{dados['modalidade']} nº {dados['numero_modalidade']}, mediante as cláusulas e condições a seguir enunciadas."
    )
    ...
    for cl in clausulas:
        ...
        is_clausula_objeto = cl["numero"].startswith("CLÁUSULA PRIMEIRA") or cl["numero"].startswith("CLÁUSULA 1") or "OBJETO" in cl["numero"]
        is_clausula_dotacao = cl["numero"].startswith("CLÁUSULA QUARTA") or cl["numero"].startswith("CLÁUSULA 4") or "DOTAÇÃO" in cl["numero"]
        ...
        if is_clausula_objeto:
            ...
            for idx_p, par in enumerate(cl["conteudo"]):
                ...
                if par.startswith("1.2.") or (len(cl["conteudo"]) == 2 and idx_p == 0):
                    ...
                    if tab_din and tab_din.get("headers") and tab_din.get("rows"):
                        ...
                        num_rows = len(dyn_rows) + (2 if has_tot else 1)
                        t_it = doc.add_table(rows=num_rows, cols=len(dyn_headers))
                        t_it.alignment = WD_TABLE_ALIGNMENT.CENTER
                        
                        for ci, h in enumerate(dyn_headers):
                            cell = t_it.cell(0, ci)
                            p_h = cell.paragraphs[0]
                            p_h.alignment = WD_ALIGN_PARAGRAPH.CENTER
                            rh = p_h.add_run(str(h))
                            rh.bold = True
                            rh.font.name = "Arial"
                            rh.font.size = Pt(8.5)
                        ...
                        for row_idx, r_vals in enumerate(dyn_rows, start=1):
                            for ci in range(len(dyn_headers)):
                                val_str = str(r_vals[ci]) if ci < len(r_vals) else ""
                                cell = t_it.cell(row_idx, ci)
                                p_cv = cell.paragraphs[0]
                                align_code = dyn_alignments[ci] if ci < len(dyn_alignments) else "LEFT"
                                p_cv.alignment = WD_ALIGN_PARAGRAPH.CENTER if align_code == "CENTER" else (
                                    WD_ALIGN_PARAGRAPH.RIGHT if align_code == "RIGHT" else WD_ALIGN_PARAGRAPH.JUSTIFY
                                )
                                cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                                rcv = p_cv.add_run(val_str)
                                rcv.font.name = "Arial"
                                rcv.font.size = Pt(8)

                        if has_tot:
                            tot_row = len(dyn_rows) + 1
                            if tot_col > 0:
                                t_it.cell(tot_row, 0).merge(t_it.cell(tot_row, tot_col - 1))
                            p_tot_lbl = t_it.cell(tot_row, 0).paragraphs[0]
                            p_tot_lbl.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                            rtot = p_tot_lbl.add_run("TOTAL")
                            rtot.bold = True
                            rtot.font.name = "Arial"
                            rtot.font.size = Pt(8.5)

                            p_tot_val = t_it.cell(tot_row, tot_col).paragraphs[0]
                            p_tot_val.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                            rtotv = p_tot_val.add_run(tot_val_txt)
                            rtotv.bold = True
                        ...
                        for row_idx, it_data in enumerate(itens_list, start=1):
                            row_vals = [
                                str(it_data.get("item", row_idx)),
                                str(it_data.get("descricao", dados["objeto"])),
                                str(it_data.get("quantidade", "1")),
                                str(it_data.get("unidade", "Unid.")),
                                str(it_data.get("valor_unitario", dados["valor_total_formatado"])),
                                str(it_data.get("valor_total", dados["valor_total_formatado"]))
                            ]
                            for c_col, val in enumerate(row_vals):
                                c_cell = t_it.cell(row_idx, c_col)
                                p_cv = c_cell.paragraphs[0]
                                # Make Item (0) centered, and Descricao (1) Justified.
                                p_cv.alignment = WD_ALIGN_PARAGRAPH.CENTER if c_col in [0, 2, 3] else (WD_ALIGN_PARAGRAPH.RIGHT if c_col in [4, 5] else WD_ALIGN_PARAGRAPH.JUSTIFY)
                                c_cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                                rcv = p_cv.add_run(val)
                                rcv.font.name = "Arial"
                                rcv.font.size = Pt(8)

                        tot_row = len(itens_list) + 1
                        t_it.cell(tot_row, 0).merge(t_it.cell(tot_row, 4))
                        p_tot_lbl = t_it.cell(tot_row, 0).paragraphs[0]
                        p_tot_lbl.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                        rtot = p_tot_lbl.add_run("TOTAL")
                        rtot.bold = True
                        ...
                        p_tot_val = t_it.cell(tot_row, 5).paragraphs[0]
                        p_tot_val.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                        rtotv = p_tot_val.add_run(tot_display)
                        rtotv.bold = True
                        rtotv.font.name = "Arial"
                        rtotv.font.size = Pt(8.5)
            ...
            for idx_d, par in enumerate(cl["conteudo"]):
                ...
                if idx_d == 0:
                    t_dot = doc.add_table(rows=5, cols=2)
                    t_dot.alignment = WD_TABLE_ALIGNMENT.CENTER
                    dot_items = [
                        ("Exercício", str(dados["dotacao_exercicio"])),
                        ("Poder", str(dados["dotacao_poder"])),
                        ("Órgão", str(dados["dotacao_orgao"])),
                        ("Unidade Orçamentária/atividade", str(dados["dotacao_unidade"])),
                        ("Natureza da despesa", str(dados["dotacao_natureza"]))
                    ]
...
def gerar_contrato_pdf(dados_brutos: Dict[str, Any]) -> io.BytesIO:
    ...
    texto_id = (
        ...
        f"<b>{dados['modalidade'].upper()} Nº {dados['numero_modalidade']}</b><br/>"
    ...
    preambulo_texto = (
        ...
        f"<b>Lei nº 14.133, de 1º de abril de 2021</b>, resolvem celebrar o presente Termo de Contrato, decorrente do "
        f"<b>{dados['modalidade']} nº {dados['numero_modalidade']}</b>, mediante as cláusulas e condições a seguir enunciadas."
    )
            ...
            for idx_p, p_txt in enumerate(cl["conteudo"]):
                ...
                if p_txt.startswith("1.2.") or (len(cl["conteudo"]) == 2 and idx_p == 0):
                    ...
                    if tab_din and tab_din.get("headers") and tab_din.get("rows"):
                        ...
                        dyn_rows = tab_din["rows"]
                        dyn_alignments = tab_din.get("col_alignments", ["LEFT"] * len(dyn_headers))
                        col_widths = tab_din.get("col_widths")
                        tot_col = tab_din.get("total_col_idx")
                        tot_val_txt = tab_din.get("total_formatado") or tot_display
        ...
        # Inserir tabela de dotação orçamentária na Cláusula Quarta
            ...
            for idx_d, d_txt in enumerate(cl["conteudo"]):
                ...
                if idx_d == 0:
                    story.append(Spacer(1, 3))
                    t_dot_data = [
                        [Paragraph("<b>Exercício</b>", style_tbl_td), Paragraph(str(dados["dotacao_exercicio"]), style_tbl_td)],
                        [Paragraph("<b>Poder</b>", style_tbl_td), Paragraph(str(dados["dotacao_poder"]), style_tbl_td)],
                        [Paragraph("<b>Órgão</b>", style_tbl_td), Paragraph(str(dados["dotacao_orgao"]), style_tbl_td)],
                        [Paragraph("<b>Unidade Orçamentária/atividade</b>", style_tbl_td), Paragraph(str(dados["dotacao_unidade"]), style_tbl_td)],
                        [Paragraph("<b>Natureza da despesa</b>", style_tbl_td), Paragraph(str(dados["dotacao_natureza"]), style_tbl_td)]
                    ]
