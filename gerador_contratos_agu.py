...
def aplicar_estilo_tabela_oficial_docx(t_it, col_widths_pt=None):
    """
    Aplica bordas elegantes, margens internas (padding), quebra de texto,
    auto-ajuste de colunas e propriedades de repetição de cabeçalho à tabela DOCX.
    """
    t_it.alignment = WD_TABLE_ALIGNMENT.CENTER
    t_it.autofit = False

    # Bordas elegantes em todas as células
    ...
    # Ajuste de texto às bordas: margens internas (padding para o texto respirar)
    cell_mar = parse_xml(
        r'<w:tblCellMar {} >'
    ...
    # Repetir cabeçalho no topo de cada página
    if len(t_it.rows) > 0:
        t_it.rows[0]._tr.get_or_add_trPr().append(parse_xml(r'<w:tblHeader {}/>'.format(nsdecls('w'))))

    # Largura útil padrão A4 para tabela (~480 pt = ~16.9 cm)
        ...
        if is_clausula_objeto:
            ...
            for idx_d, par in enumerate(cl["conteudo"]):
                ...
                if idx_d == 0:
                    ...
                    t_dot = doc.add_table(rows=len(dot_items), cols=2)
