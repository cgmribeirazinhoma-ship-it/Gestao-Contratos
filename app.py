...
def tela_login():
    ...
    with col2:
        ...
        with st.form("login_form_gel"):
            ...
            usuario_login = st.text_input("NOME DE USUÁRIO", placeholder="Ex: admin, operador ou seu usuário", key="input_login_usuario")
            ...
            if submit:
                if not usuario_login or not senha:
                    st.warning("⚠️ Por favor, informe seu nome de usuário e senha.")
                else:
    ...
    with tab_m3:
        ...
        with st.expander("📊 Importar Planilha ou PDF de Itens (Auto-Ajustável com IA)", expanded=True):
            ...
            if tab_din_cgutec and tab_din_cgutec.get("headers") and tab_din_cgutec.get("rows"):
                ...
                c_bot_ia1, c_bot_ia2, c_bot_limp = st.columns([1.5, 1.2, 1.2])
                with c_bot_ia1:
        ...
        with st.expander("🏢 Dados da Empresa Contratada (Fornecedor)", expanded=True):
            ...
            def _callback_buscar_cnpj_btn():
                _executar_busca_cnpj_banco(st.session_state.get("cgutec_forn_cnpj", ""))

            def _callback_puxar_api_receita():
                cnpj_pend = st.session_state.get("cgutec_forn_cnpj", "") or st.session_state.get("cgutec_cnpj_nao_cadastrado", "")
                c_limpo = re.sub(r"\D", "", str(cnpj_pend or ""))
                if not c_limpo:
                    st.session_state["cgutec_cnpj_status"] = "incompleto"
                    st.session_state["cgutec_cnpj_msg"] = "Por favor, digite o CNPJ no campo abaixo antes de buscar via API."
                    return
                if len(c_limpo) != 14:
                    st.session_state["cgutec_cnpj_status"] = "incompleto"
                    st.session_state["cgutec_cnpj_msg"] = f"CNPJ incompleto ({len(c_limpo)} dígitos). Um CNPJ válido possui 14 dígitos numéricos."
                    return
                res_api = puxar_fornecedor_api_e_cadastrar(c_limpo, engine)
                if res_api.get("sucesso"):
                    st.session_state["cgutec_forn_cnpj"] = res_api.get("cnpj_formatado") or formatar_cnpj_cpf(c_limpo)
                    ...
                    st.session_state["cgutec_cnpj_msg"] = f"Dados da empresa **{res_api.get('razao_social')}** obtidos com sucesso na Receita Federal ({res_api.get('fonte', 'API Oficial')}) e sincronizados no sistema!"
                else:
                    st.session_state["cgutec_cnpj_status"] = "erro_api"
                    st.session_state["cgutec_cnpj_msg"] = f"Não foi possível obter dados via API da Receita: {res_api.get('erro', 'CNPJ não encontrado ou serviço indisponível.')}"
            ...
            # 2. Campo Inteligente de CNPJ com Busca Direta via API e no Banco
            c_cnpj_inp, c_cnpj_btn_api, c_cnpj_btn_banco = st.columns([2.3, 1.3, 0.9])
            with c_cnpj_inp:
                forn_cnpj_val = st.text_input(
                    "CNPJ da Contratada (Ao digitar, busca no banco ou clique para puxar via API):",
                    key="cgutec_forn_cnpj",
                    placeholder="00.000.000/0001-00 (ou somente números)",
                    on_change=_callback_cnpj_input,
                    help="Informe o CNPJ da empresa e clique em 'Puxar via API' para consultar na Receita Federal."
                )
            with c_cnpj_btn_api:
                st.write("")
                st.write("")
                st.button(
                    "🌐 Puxar via API",
                    key="btn_puxar_api_receita_m3",
                    on_click=_callback_puxar_api_receita,
                    use_container_width=True,
                    type="primary",
                    help="Consulta dados cadastrais oficiais diretamente na Receita Federal (BrasilAPI/MinhaReceita/ReceitaWS)."
                )
            with c_cnpj_btn_banco:
                st.write("")
                st.write("")
                st.button(
                    "⚡ No Banco",
                    key="btn_verificar_cnpj_cgutec",
                    on_click=_callback_buscar_cnpj_btn,
                    use_container_width=True,
                    type="secondary",
                    help="Busca os dados cadastrados previamente no banco de dados local."
                )
            ...
            if status_cnpj == "encontrado_banco":
                ...
                c_btn_enri, _ = st.columns([1.8, 2])
                with c_btn_enri:
                    st.button(
                        "🔄 Atualizar Dados Oficiais via API (Receita Federal)",
                        key="btn_enriquecer_api_m3",
                        on_click=_callback_puxar_api_receita,
                        use_container_width=True,
                        help="Atualiza endereço e representante legal com os dados oficiais da Receita Federal."
                    )
            elif status_cnpj == "encontrado_api":
                st.success(f"🎉 {msg_cnpj}")
            elif status_cnpj == "nao_encontrado_banco":
                st.warning(f"⚠️ {msg_cnpj} Clique em **'🌐 Puxar via API'** acima para buscar na Receita Federal.")
            elif status_cnpj == "erro_api":
                st.error(f"❌ {msg_cnpj}")
                c_btn_api_retry, _ = st.columns([1.6, 2])
                with c_btn_api_retry:
                    st.button(
                        "🌐 Tentar Novamente via API",
                        type="primary",
                        key="btn_retry_api_receita_m3",
                        on_click=_callback_puxar_api_receita,
                        use_container_width=True
                    )
...
def tela_fornecedores():
    ...
    with aba_novo:
        ...
        c_tf_c1, c_tf_c2, c_tf_c3 = st.columns([2.3, 1.2, 0.9])
        with c_tf_c1:
            cnpj_tf_in = st.text_input("CNPJ para Consulta:", placeholder="00.000.000/0001-00 (ou somente números)", key="tf_cnpj_busca")
        with c_tf_c2:
            st.write("")
            st.write("")
            btn_tf_api = st.button("🌐 Puxar via API", key="btn_tf_puxar_api_btn", use_container_width=True, type="primary")
        with c_tf_c3:
            st.write("")
            st.write("")
            btn_tf_banco = st.button("⚡ No Banco", key="btn_tf_consultar_cnpj", use_container_width=True)

        if btn_tf_api and cnpj_tf_in:
            c_l = re.sub(r"\D", "", cnpj_tf_in)
            if len(c_l) == 14:
                with st.spinner("Puxando dados oficiais via Receita Federal..."):
                    ...
                    if res_a.get("sucesso"):
                        ...
                        st.success(f"🎉 Dados da empresa **{res_a.get('razao_social')}** obtidos via Receita Federal ({res_a.get('fonte', 'API')}) e sincronizados com sucesso!")
                        st.rerun()
                    else:
        ...
        if btn_tf_banco and cnpj_tf_in:
            c_l = re.sub(r"\D", "", cnpj_tf_in)
            if len(c_l) == 14:
                ...
                if res_b:
                    ...
                    st.info(f"✅ Empresa encontrada no banco de dados: **{res_b.get('razao_social')}**")
                else:
                    st.warning(f"⚠️ O CNPJ {formatar_cnpj_cpf(c_l)} não consta no banco de dados local. Clique em '🌐 Puxar via API' para buscar na Receita Federal.")
            else:
                st.warning("⚠️ Digite um CNPJ válido com 14 dígitos.")
