...
def verificar_login(login_input, senha):
    ...
    if login_clean:
        login_clean = login_clean[0].lower() + login_clean[1:].lower()
...
def cadastrar_novo_usuario(nome, usuario_login=None, senha="", cargo="OPERADOR"):
    ...
    if not usuario_login or not str(usuario_login).strip():
        ...
        if len(slug_partes) > 1:
            login_limpo = f"{re.sub(r'[^a-z0-9]', '', slug_partes[0])}.{re.sub(r'[^a-z0-9]', '', slug_partes[-1])}"
        else:
            login_limpo = re.sub(r'[^a-z0-9]', '', slug)
        if login_limpo:
            login_limpo = login_limpo[0].lower() + login_limpo[1:].lower()
    else:
        login_bruto = str(usuario_login).strip()
        if "@" in login_bruto:
            login_bruto = login_bruto.split("@")[0]
        if login_bruto:
            login_bruto = login_bruto[0].lower() + login_bruto[1:].lower()
        login_limpo = re.sub(r'[^a-z0-9._-]', '', login_bruto)

    if not login_limpo:
        return False, "Nome de usuário inválido. Utilize letras, números, ponto ou traço."
...
def atualizar_usuario(usuario_id, nome, usuario_login, cargo, ativo, nova_senha=None):
    if not nome or not str(nome).strip():
        return False, "O campo 'Nome Completo' é obrigatório."
    nome_limpo = str(nome).strip()
    login_bruto = str(usuario_login).strip() if usuario_login else ""
    if "@" in login_bruto:
        login_bruto = login_bruto.split("@")[0]
    if login_bruto:
        login_bruto = login_bruto[0].lower() + login_bruto[1:].lower()
...
def tela_login():
    ...
    <style>
        ...
        /* Garante que o campo de nome de usuário seja exibido sempre em minúsculas */
        ...
        with st.form("login_form_gel"):
            ...
            if submit:
                if not usuario_login or not senha:
                    ...
                    # Converte a primeira letra (e o login) para minúscula
                    login_tratado = usuario_login.strip()
                    if login_tratado:
                        login_tratado = login_tratado[0].lower() + login_tratado[1:].lower()
        ...
        if u_sel:
            ...
            with st.form(f"form_edit_u_{uid}"):
                ...
                with c1:
                    ...
                    if val_login_inicial:
                        val_login_inicial = val_login_inicial[0].lower() + val_login_inicial[1:].lower()
    ...
    with tab_cadastro:
        with st.form("form_novo_user_gel"):
            ...
            if st.form_submit_button("➕ Criar Acesso", type="primary", use_container_width=True):
                ...
                if user_cad_tratado:
                    user_cad_tratado = user_cad_tratado[0].lower() + user_cad_tratado[1:].lower()
